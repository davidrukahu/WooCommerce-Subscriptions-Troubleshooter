...
class WCST_Discrepancy_Detector {
    ...
    private function check_missing_actions($subscription) {
        ...
        if ($expected_renewal_date) {
            $renewal_actions = $wpdb->get_var($wpdb->prepare("
                SELECT COUNT(*) FROM {$actions_table}
                WHERE hook LIKE '%renewal%'
                AND args LIKE %s
                AND scheduled_date >= %s
                AND status IN ('pending', 'completed')
            ", '%' . $wpdb->esc_like($subscription_id) . '%', $expected_renewal_date));
        ...
        $failed_actions = $wpdb->get_var($wpdb->prepare("
            SELECT COUNT(*) FROM {$actions_table}
            WHERE args LIKE %s
            AND status = 'failed'
    ...
    private function prioritize_discrepancies($discrepancies) {
        ...
        usort($discrepancies, function($a, $b) use ($severity_order) {
