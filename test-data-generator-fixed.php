...
class WCST_Test_Data_Generator {
	...
	private function create_single_subscription( $customer_id, $product_id, $scenario ) {
		...
		$subscription->set_date( 'date_created', date( 'Y-m-d H:i:s', $start_date ) );
		...
		if ( $trial_length > 0 ) {
			...
			$subscription->set_date( 'trial_end', date( 'Y-m-d H:i:s', $trial_end ) );
		}
		
		// Set next payment date
		if ( 'cancelled' !== $scenario['status'] ) {
			$next_payment = $this->calculate_next_payment_date( $subscription, $start_date );
			$subscription->set_date( 'next_payment', date( 'Y-m-d H:i:s', $next_payment ) );
		}
	...
	private function create_failed_payment_scenario( $subscription ) {
		...
		$subscription->set_date( 'last_payment', $last_payment );
		$subscription->save();
	}
